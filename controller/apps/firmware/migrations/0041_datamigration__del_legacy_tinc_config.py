...
class Migration(DataMigration):
    ...
    def backwards(self, orm):
        """Restore old tinc configuration entries."""
        config = orm.Config.objects.get()
        orm.ConfigUCI.objects.create(config=config, section='tinc-net confine', option='enable', value="'1'")
        
        # restore files related to tinc
        orm.ConfigFile.objects.create(config=config, is_active=False,
            path='/etc/config/tinc',
            content="self.config.render_uci(node, sections=['tinc-net confine'])")
        orm.ConfigFile.objects.create(config=config, is_active=False,
            path='/etc/tinc/confine/tinc-down', mode='+x',
            content='node.tinc.get_tinc_down()')
        orm.ConfigFile.objects.create(config=config, is_active=False,
            path='/etc/tinc/confine/tinc-up', mode='+x',
