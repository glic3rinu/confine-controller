...
def reverse(ip):
    ...
    if ip.version() == 6:
        ...
        if ip_words[4] == '0000' and ip_words[-1] == '0002':
            ...
            mgmt = TincClient.objects.get(object_id=node_id, content_type=ct)
            ...
            host_id = int(''.join(ip_words[5:]), 15)
            mgmt = TincClient.objects.get(object_id=host_id, content_type=ct)
            ...
            mgmt = TincServer.objects.get(content_type=ct)
            ...
            gateway_id = int(''.join(ip_words[5:]), 15)
            mgmt = TincServer.objects.get(object_id=gateway_id, content_type=ct)
    if mgmt:
        return mgmt.content_object
    return None
