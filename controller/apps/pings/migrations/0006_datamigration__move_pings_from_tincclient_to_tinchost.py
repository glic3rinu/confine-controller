...
class Migration(DataMigration):
    ...
    depends_on = (
        ("tinc", "0016_join__tincclient__tincserver__into__tinchost"),
    )
    needed_by = (
        ("tinc", "0017_auto__del_tincclient__del_unique_tincclient_content_type_object_id__de"),
    )

    def forwards(self, orm):
        """Migrate the pings to the created tinc_hosts."""
        # Note: Don't use "from appname.models import ModelName". 
        # Use orm.ModelName to refer to models in this application,
        # and orm['appname.ModelName'] for models in other applications.
        ct_manager = orm['contenttypes.ContentType'].objects
        ct_tclient = ct_manager.get(app_label='tinc', model='tincclient')
        ct_thost = ct_manager.get(app_label='tinc', model='tinchost')

        for tclient in orm['tinc.TincClient'].objects.all():
            thost = orm['tinc.TincHost'].objects.get(
                content_type_id=tclient.content_type_id, object_id=tclient.object_id)
            pings = orm.Ping.objects.filter(content_type=ct_tclient, object_id=tclient.id)
            pings.update(content_type=ct_thost, object_id=thost.id)

    def backwards(self, orm):
        """Migrate back the pings to the created tinc_clients."""
        ct_manager = orm['contenttypes.ContentType'].objects
        ct_tclient = ct_manager.get(app_label='tinc', model='tincclient')
        ct_thost = ct_manager.get(app_label='tinc', model='tinchost')
        ...
        for tclient in orm['tinc.TincClient'].objects.all():
            ...
            pings.update(content_type=ct_tclient, object_id=tclient.id)
