...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """XOR merge of sliver data and sliver overlay."""
        # Note: Don't use "from appname.models import ModelName". 
        # Use orm.ModelName to refer to models in this application,
        # and orm['appname.ModelName'] for models in other applications.
        
        ## check if we can apply this migration ##
        odd_slices = orm.Slice.objects.exclude(sliver_defaults__data='', sliver_defaults__data_uri='').exclude(sliver_defaults__overlay='', sliver_defaults__overlay_uri='').values('pk')
        odd_slivers = orm.Sliver.objects.exclude(data='', data_uri='').exclude(overlay='', overlay_uri='').values('pk')
        
        if len(odd_slices) or len(odd_slivers):
            raise RuntimeError("Cannot perform this migration. Some objects "
                               "have both attributes 'data' and 'overlay': "
                               "slices %s, slivers %s. Please manually remove "
                               "one of them to continue." % (odd_slices, odd_slivers))
        
        ## migrate data: overlay -> data  ##
        orm.SliverDefaults.objects.exclude(overlay='').update(data=F('overlay'), data_sha256=F('overlay_sha256'))
        orm.SliverDefaults.objects.exclude(overlay_uri='').update(data_uri=F('overlay_uri'), data_sha256=F('overlay_sha256'))
        assert not orm.SliverDefaults.objects.exclude(data='').filter(data_sha256='').exists(), "Some slice with data but not SHA256!"

        orm.Sliver.objects.exclude(overlay='').update(data=F('overlay'), data_sha256=F('overlay_sha256'))
        orm.Sliver.objects.exclude(overlay_uri='').update(data_uri=F('overlay_uri'), data_sha256=F('overlay_sha256'))
        
        assert not orm.Sliver.objects.exclude(data='').filter(data_sha256='').exists(), "Some sliver with data but not SHA256!"
        
        ## remove overlay data ##
        orm.SliverDefaults.objects.all().update(overlay='', overlay_uri='', overlay_sha256='')
        orm.Sliver.objects.all().update(overlay='', overlay_uri='', overlay_sha256='')

    def backwards(self, orm):
        """Nothing TBD as we cannot guess how to revert forwards operation."""
        warnings.warn("Data unchanged, migration 'slices.0024' cannot guess previous state.")
    ...
    models = {
        ...
        u'slices.slice': {
            ...
            'description': ('django.db.models.fields.TextField', [], {'blank': 'True'}),
            'expires_on': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2014, 12, 7, 0, 0)', 'null': 'True', 'blank': 'True'}),
