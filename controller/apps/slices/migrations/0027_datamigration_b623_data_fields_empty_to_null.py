...
class Migration(DataMigration):
    # Thanks Django for having the good design convention to store
    # null strings as an empty one '' and allow having two different
    # cases on the DB when both blank and null are allowed! (irony)
    ...
    def forwards(self, orm):
        ...
        orm.Sliver.objects.filter(data_sha256='').update(data_sha256=None)

    def backwards(self, orm):
        "Convert null to empty string."
        orm.SliverDefaults.objects.filter(data_uri__isnull=True).update(data_uri='')
        orm.SliverDefaults.objects.filter(data_sha256__isnull=True).update(data_sha256='')
        orm.Sliver.objects.filter(data_uri__isnull=True).update(data_uri='')
        orm.Sliver.objects.filter(data_sha256__isnull=True).update(data_sha256='')
