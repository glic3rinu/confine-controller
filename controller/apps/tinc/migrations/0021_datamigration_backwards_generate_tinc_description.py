...
class Migration(DataMigration):
    ...
    def backwards(self, orm):
        "Filling host description if it's null."
        # orm doesn't work using F objects queries
        #is_empty = Q(description__isnull=True) | Q(description='')
        #orm.Host.objects.filter(is_empty).update(description="host %s" % F('id'))
        db.execute("UPDATE tinc_host SET description = 'host ' || id WHERE description = '' OR description IS NULL")
