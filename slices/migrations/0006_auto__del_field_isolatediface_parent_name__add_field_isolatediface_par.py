...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'IsolatedIface', fields ['parent_name', 'sliver']
        db.delete_unique('slices_isolatediface', ['parent_name', 'sliver_id'])

        # Deleting field 'IsolatedIface.parent_name'
        db.delete_column('slices_isolatediface', 'parent_name')

        # Adding field 'IsolatedIface.parent'
        db.add_column('slices_isolatediface', 'parent',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['nodes.Node'], null=True, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'IsolatedIface', fields ['parent', 'sliver']
        db.create_unique('slices_isolatediface', ['parent_id', 'sliver_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'IsolatedIface', fields ['parent', 'sliver']
        db.delete_unique('slices_isolatediface', ['parent_id', 'sliver_id'])

        # Adding field 'IsolatedIface.parent_name'
        db.add_column('slices_isolatediface', 'parent_name',
                      self.gf('django.db.models.fields.CharField')(default='eth0', max_length=16),
                      keep_default=False)

        # Deleting field 'IsolatedIface.parent'
        db.delete_column('slices_isolatediface', 'parent_id')

        # Adding unique constraint on 'IsolatedIface', fields ['parent_name', 'sliver']
        db.create_unique('slices_isolatediface', ['parent_name', 'sliver_id'])
    ...
    models = {
        ...
        'slices.isolatediface': {
            ...
            'parent': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['nodes.Node']", 'null': 'True', 'blank': 'True'}),
