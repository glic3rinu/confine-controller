...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing M2M table for field interfaces on 'Sliver'
        db.delete_table('slices_sliver_interfaces')

        # Adding field 'NetworkRequest.interface'
        db.add_column('slices_networkrequest', 'interface',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name='network_requests', null=True, to=orm['nodes.Interface']),
                      keep_default=False)

    def backwards(self, orm):
        # Adding M2M table for field interfaces on 'Sliver'
        db.create_table('slices_sliver_interfaces', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('sliver', models.ForeignKey(orm['slices.sliver'], null=False)),
            ('interface', models.ForeignKey(orm['nodes.interface'], null=False))
        ))
        db.create_unique('slices_sliver_interfaces', ['sliver_id', 'interface_id'])

        # Deleting field 'NetworkRequest.interface'
        db.delete_column('slices_networkrequest', 'interface_id')
